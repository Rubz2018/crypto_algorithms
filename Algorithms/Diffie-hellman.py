...
def generate_private_key(n,g):
    ...
    k2 = pow(g,y,n)
    '''
    if an attacker wants to get x and y  (the private keys) which is the discrete logarithm 
    problem it is an exponentially slow process!!!
    '''
    print("Alice's private key %s" % (pow(k2,x,n)))
    print("Bob's private key %s" % (pow(k1,y,n)))

if __name__ == '__main__':
    # it shhould be a huge prime number
    n=37
